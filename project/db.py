...
class User(Base):
  ...
  def __repr__(self):
    return "<User(username='%s', password='%s', salt='%s', emailAddress='%s')>" % (self.username, self.password, self.salt, self.emailAddress)
...
Table('users', MetaData(bind=None),
    ...
    Column('emailAddress', String(60), nullable=False), schema=None)
